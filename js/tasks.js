...
window.updateTaskProjectFilter = function () {
    ...
    let html = '<option value="">選擇專案</option>';
...
function renderTasksList(draggable = false) {
    ...
    if (container.classList.contains('active')) {
        ...
        tasks.forEach(t => {
            ...
            const displayTaskName = custName ? `${custName}_${t.taskName || ''}` : (t.taskName || '');
...
window.submitTaskEditor = async function (event) {
    ...
    try {
        ...
        if (!taskDate || !taskTime) {
            ...
            if (!taskDate) taskDate = `${yyyy}-${mm}-${dd}`;
            if (!taskTime) taskTime = `${hh}:${min}`;
        }
        ...
        if (!task.taskName) {
            document.getElementById('taskError').innerText = '請填寫任務內容';
            return;
        }
